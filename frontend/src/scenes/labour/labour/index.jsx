...
import labourSchema from "../../../data/schemas/labourSchema";
...
const Labour = () => {
    ...
    const [labourEdits, setLabourEdits] = useSessionStorage('labourEdits', []);
    const [newLabourEdit, setNewLabourEdit] = useSessionStorage('newLabourEdit', undefined);


    const getValuesForId = (id) => {
        try {
            return labour.filter(o => (
                o
    ...
    const handleSave = (values) => {
        ...
        const {description, labourType} = values;
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                if(index === -1) {
                    if((labour || []).map((p) => p.labourType.toLowerCase()).includes(labourType.toLowerCase())) {
            ...
            if(typeof(values.id) !== 'undefined') {
                useLabourId = values.id;
                window.alert(`Labour updated for preset # ${useLabourId} with description:\n ${values.description} and type ${labourType}`);
            } else {
                useLabourId = savedId;
                window.alert(`Labour updated for preset # ${useLabourId} with description:\n ${values.description} and type ${labourType}`);
            }
            ...
            log(`Error saving labour`);
            console.log(e);
            window.alert(`Error saving labour: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useLabourId = handleSave(values);

        if(typeof(useLabourId) !== 'undefined') {
            _navigate(useLabourId);
        }
    }
    ...
    const updateEditForId = (editId, values) => {
        ...
        try {
            for(let i = 0; i < labourEdits.length; i++) {
                if(labourEdits[i].id === editId) {
                    labourEdits[i] = {...values};
                    foundEdit = true;
                    break;
                }
            }
        ...
        navigate(
            route,
            {
                state: {
                    ...
                    origin: 'labour',  // origin - will return here on save
    ...
    const handleEditClass = (labourType, values) => {
        const {id} = labourTypes.filter(cls => cls.labourType === labourType)[0] || {};
        log(`Editing labour type: ${id}`);
        handleEditSettings(id, values, '/labour-type');
    }

    const handleDuplicateClass = (labourType, values) => {
        const {id} = labourTypes.filter(cls => cls.labourType === labourType)[0] || {};
        log(`Duplicating labour type: ${id}`);
        handleDuplicateSettings(id, values, '/labour-type');
    }

    const handleCreateClass = (values) => {
        log(`Creating new labour type`);
        handleCreateSettings(values, '/labour-type')
    }
    ...
    const labourTypeOptions = labourTypes.map((_option) => {
        return {
            value: _option.id,
            label: _option.labourType,
        }
    });

    return(
        <Box m="20px" minHeight="85vh">
            <Header title="LABOUR RESOURCE" subtitle="Create / edit labour items" />
                        ...
                        >
                            ...
                            <FormControl sx={{
                                ...
                                <Select
                                    ...
                                    name="labourType"
                                    value={(labourTypeOptions ? labourTypeOptions.find(option => option.label === values.labourType) : '')}
                                    onChange={(option) => {
                                        setFieldValue( 'labourType', option.label, false  );
                                    }}
                                    onBlur={handleBlur}
                                    styles={customStyles}
                                />
                            ...
                            >
                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleEditClass(values.labourType, values) }}>
                                        EDIT
                                    </Button>
                                </Box>

                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleDuplicateClass(values.labourType, values) }}>
                                        COPY
                                    </Button>
                                </Box>
