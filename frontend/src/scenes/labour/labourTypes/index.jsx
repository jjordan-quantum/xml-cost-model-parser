...
const LabourTypes = () => {
    ...
    const handleEdit = (id) => {
        ...
        navigate(
            '/labour-type',
            { state: { loadId: id, loadMode: 'edit', origin: 'labour-types' } }
        );
    }
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const labourTypesClone = [...labourTypes];

            for(let i = 0; i < labourTypesClone.length; i++) {
                const { id } = labourTypesClone[i];
            ...
            if(index !== -1) {
                console.log(labourTypesClone);
                labourTypesClone.splice(index, 1);
                setLabourTypes(labourTypesClone);
            }
    ...
    const handleDuplicate = (id) => {
        ...
        navigate(
            '/labour-type',
            { state: { loadId: id, loadMode: 'duplicate', origin: 'labour-types' } }
        );
    }

    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR LABOUR TYPES?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setLabourTypes([]);
            window.alert(`Storage for labour types reset back to defaults`);
