...
const LabourType = () => {
    const componentName = 'LabourType';
    ...
    const [labourTypes, setLabourTypes] = useSessionStorage('labourTypes', []);
    const [labourClasses, setLabourClasses] = useSessionStorage('labourClasses', []);
    const [labourTypeEdits, setLabourTypeEdits] = useSessionStorage('labourTypeEdits', []);
    const [newLabourTypeEdit, setNewLabourTypeEdit] = useSessionStorage('newLabourTypeEdit', undefined);

    const getValuesForId = (id) => {
        try {
            return labourTypes.filter(o => (
                o
    ...
    const handleSave = (values) => {
        ...
        const {description, labourType, labourClass} = values;
        ...
        if(!labourType) {
            log(`labourType undefined - cannot save settings`);
            window.alert(`labourType undefined - cannot save settings`);
            return;
        }
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                if(index === -1) {
                    if((labourTypes || []).map((p) => p.labourType.toLowerCase()).includes(labourType.toLowerCase())) {
                        log(`labourType must be unique - already have an entry for:\n\n${labourType}`);
                        window.alert(`labourType must be unique - already have an entry for:\n\n${labourType}`);
                        return;
                    }

                    labourTypes.push({...values});
                } else {
                    labourTypes[index] = {...values};
                }
            } else {
                let maxId = -1;
            ...
            setLabourTypes(labourTypes);
            let useLabourTypeId;

            if(typeof(values.id) !== 'undefined') {
                useLabourTypeId = values.id;
                window.alert(`Labour Types updated for preset # ${useLabourTypeId} with description:\n ${values.description} and type ${labourType}`);
            } else {
                useLabourTypeId = savedId;
                window.alert(`Labour Types updated for preset # ${useLabourTypeId} with description:\n ${values.description} and type ${labourType}`);
            }

            return useLabourTypeId;
        } catch(e) {
            log(`Error saving labour type`);
            console.log(e);
            window.alert(`Error saving labour type: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const updateEditForId = (editId, values) => {
        ...
        try {
            for(let i = 0; i < labourTypeEdits.length; i++) {
                if(labourTypeEdits[i].id === editId) {
                    labourTypeEdits[i] = {...values};
                    foundEdit = true;
                    break;
                }
            }
            ...
            if(!foundEdit) {
                labourTypeEdits.push({
                    ...values,
                    id: editId,
                });
            }

            setLabourTypeEdits(labourTypeEdits);
        } catch(e) {
        ...
        navigate(
            route,
            {
                state: {
                    ...
                    origin: 'labour-type',  // origin - will return here on save
    ...
    const handleEditClass = (labourClass, values) => {
        const {id} = labourClasses.filter(cls => cls.labourClass === labourClass)[0] || {};
        log(`Editing labour class: ${id}`);
        handleEditSettings(id, values, '/labour-class');
    }

    const handleDuplicateClass = (labourClass, values) => {
        const {id} = labourClasses.filter(cls => cls.labourClass === labourClass)[0] || {};
        log(`Duplicating labour class: ${id}`);
        handleDuplicateSettings(id, values, '/labour-class');
    }

    const handleCreateClass = (values) => {
        log(`Creating new labour class`);
        handleCreateSettings(values, '/labour-class')
    }
    ...
    const labourClassOptions = labourClasses.map((_option) => {
        return {
            value: _option.id,
            label: _option.labourClass,
        }
    });

    return(
        <Box m="20px" minHeight="85vh">
            <Header title="LABOUR TYPE" subtitle="Create / edit labour types" />
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        >
                            ...
                            <TextField
                                ...
                                name="labourType"
                                error={!!touched.labourType && !!errors.labourType}
                                helperText={touched.labourType && errors.labourType}
                                sx={{ gridColumn: "span 4" }}
                            />
                            ...
                            <FormControl sx={{
                                ...
                                <Select
                                    options={labourClassOptions}
                                    name="labourClass"
                                    value={(labourClassOptions ? labourClassOptions.find(option => option.label === values.labourClass) : '')}
                                    onChange={(option) => {
                                        setFieldValue( 'labourClass', option.label, false  );
                                    }}
                                    onBlur={handleBlur}
                                    styles={customStyles}
                                />
                            ...
                            >
                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleEditClass(values.labourClass, values) }}>
                                        EDIT
                                    </Button>
                                </Box>

                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleDuplicateClass(values.labourClass, values) }}>
                                        COPY
                                    </Button>
                                </Box>
