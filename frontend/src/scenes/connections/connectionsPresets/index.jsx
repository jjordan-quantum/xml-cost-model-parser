...
const ConnectionsPresets = () => {
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const connectionsClone = [...connections];

            for(let i = 0; i < connectionsClone.length; i++) {
                const { id } = connectionsClone[i];
            ...
            if(index !== -1) {
                console.log(connectionsClone);
                connectionsClone.splice(index, 1);
                setConnections(connectionsClone);
            }
    ...
    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR CONNECTIONS?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setConnections([]);
            window.alert(`Storage for connections reset back to defaults`);
    ...
    const columns = [
        ...
        {
            field: "wssProvider",
            headerName: "WebSocket Provider",
            flex: 1,
        },
        {
            field: "httpsProvider",
            headerName: "HTTPS Provider",
            flex: 1,
        },
        {
            field: "httpsProviderFlashbots",
            headerName: "MEV HTTPS Provider",
            flex: 1,
        },
        // {
    ...
    return (
        <Box m="20px">
            <Header title="CONNECTIONS PRESETS" subtitle="List of existing presets for Connections"/>
...
export default ConnectionsPresets;
