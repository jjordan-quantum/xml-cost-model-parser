...
import subcontractTypeSchema from "../../../data/schemas/subcontractTypeSchema";
...
const SubcontractType = () => {
    const componentName = 'SubcontractType';
    ...
    const getValuesForId = (id) => {
        try {
            return subcontractTypes.filter(o => (
                o
    ...
    const handleSave = (values) => {
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                for(let i = 0; i < subcontractTypes.length; i++) {
                    const { id } = subcontractTypes[i];
                ...
                if(index === -1) {
                    subcontractTypes.push({...values});
                } else {
                    subcontractTypes[index] = {...values};
                }
            } else {
                let maxId = -1;

                for(const item of subcontractTypes) {
            ...
            setSubcontractTypes(subcontractTypes);
            let useSubcontractTypeId;

            if(typeof(values.id) !== 'undefined') {
                useSubcontractTypeId = values.id;
                window.alert(`Subcontract Types updated for preset # ${useSubcontractTypeId} with description:\n ${values.description}`);
            } else {
                useSubcontractTypeId = savedId;
                window.alert(`Subcontract Types updated for preset # ${useSubcontractTypeId} with description:\n ${values.description}`);
            }

            return useSubcontractTypeId;
        } catch(e) {
            log(`Error saving subcontract type`);
            console.log(e);
            window.alert(`Error saving subcontract type: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useSubcontractTypeId = handleSave(values);

        if(typeof(useSubcontractTypeId) !== 'undefined') {
            _navigate(useSubcontractTypeId);
        }
    }
    ...
    return(
        <Box m="20px" minHeight="85vh">
            <Header title="SUBCONTRACT TYPE" subtitle="Create / edit subcontract types" />

            <Formik
                onSubmit={handleFormSubmit}
                initialValues={getInitialValues(loadId, loadMode)}
                validationSchema={subcontractTypeSchema}
            >
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        >
                            ...
                            <TextField
                                ...
                                onBlur={handleBlur}
                                onChange={handleChange}
                                value={values.subcontractType}
                                name="subcontractType"
                                error={!!touched.subcontractType && !!errors.subcontractType}
                                helperText={touched.subcontractType && errors.subcontractType}
                                sx={{ gridColumn: "span 4" }}
                            />
...
export default SubcontractType;
