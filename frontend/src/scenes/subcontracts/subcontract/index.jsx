...
import subcontractSchema from "../../../data/schemas/subcontractSchema";
...
const Subcontract = () => {
    ...
    const [subcontractEdits, setSubcontractEdits] = useSessionStorage('subcontractEdits', []);
    const [newSubcontractEdit, setNewSubcontractEdit] = useSessionStorage('newSubcontractEdit', undefined);

    const getValuesForId = (id) => {
        try {
            return subcontracts.filter(o => (
                o
    ...
    const handleSave = (values) => {
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                if(index === -1) {
                    subcontracts.push({...values});
                } else {
                    subcontracts[index] = {...values};
                }
            } else {
                let maxId = -1;
            ...
            let useSubcontractId;

            if(typeof(values.id) !== 'undefined') {
                useSubcontractId = values.id;
                window.alert(`Subcontracts updated for preset # ${useSubcontractId} with description:\n ${values.description}`);
            } else {
                useSubcontractId = savedId;
                window.alert(`Subcontracts updated for preset # ${useSubcontractId} with description:\n ${values.description}`);
            }

            return useSubcontractId;
        } catch(e) {
            log(`Error saving subcontract`);
            console.log(e);
            window.alert(`Error saving subcontract: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useSubcontractId = handleSave(values);

        if(typeof(useSubcontractId) !== 'undefined') {
            _navigate(useSubcontractId);
        }
    }
    ...
    const updateEditForId = (editId, values) => {
        ...
        try {
            for(let i = 0; i < subcontractEdits.length; i++) {
                if(subcontractEdits[i].id === editId) {
                    subcontractEdits[i] = {...values};
                    foundEdit = true;
                    break;
                }
            }
            ...
            if(!foundEdit) {
                subcontractEdits.push({
                    ...values,
                    id: editId,
                });
            }

            setSubcontractEdits(subcontractEdits);
        } catch(e) {
        ...
        navigate(
            route,
            {
                state: {
                    ...
                    origin: 'subcontract',  // origin - will return here on save
    ...
    const handleEditClass = (subcontractType, values) => {
        const {id} = subcontractTypes.filter(cls => cls.subcontractType === subcontractType)[0] || {};
        log(`Editing subcontract type: ${id}`);
        handleEditSettings(id, values, '/subcontract-type');
    }

    const handleDuplicateClass = (subcontractType, values) => {
        const {id} = subcontractTypes.filter(cls => cls.subcontractType === subcontractType)[0] || {};
        log(`Duplicating subcontract type: ${id}`);
        handleDuplicateSettings(id, values, '/subcontract-type');
    }

    const handleCreateClass = (values) => {
        log(`Creating new subcontract type`);
        handleCreateSettings(values, '/subcontract-type')
    }
    ...
    const subcontractTypeOptions = subcontractTypes.map((_option) => {
        return {
            value: _option.id,
            label: _option.subcontractType,
        }
    });

    return(
        <Box m="20px" minHeight="85vh">
            <Header title="SUBCONTRACT" subtitle="Create / edit subcontracts" />

            <Formik
                onSubmit={handleFormSubmit}
                initialValues={getInitialValues(loadId, loadMode)}
                validationSchema={subcontractSchema}
            >
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        >
                            ...
                            <TextField
                                ...
                                onBlur={handleBlur}
                                onChange={handleChange}
                                value={values.subcontract}
                                name="subcontract"
                                error={!!touched.subcontract && !!errors.subcontract}
                                helperText={touched.subcontract && errors.subcontract}
                                sx={{ gridColumn: "span 4" }}
                            />
                            ...
                            <FormControl sx={{
                                ...
                                <Select
                                    options={subcontractTypeOptions}
                                    name="subcontractType"
                                    value={(subcontractTypeOptions ? subcontractTypeOptions.find(option => option.label === values.subcontractType) : '')}
                                    onChange={(option) => {
                                        setFieldValue( 'subcontractType', option.label, false  );
                                    }}
                                    onBlur={handleBlur}
                                    styles={customStyles}
                                />
                            ...
                            >
                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleEditClass(values.subcontractType, values) }}>
                                        EDIT
                                    </Button>
                                </Box>

                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleDuplicateClass(values.subcontractType, values) }}>
                                        COPY
                                    </Button>
                                </Box>
