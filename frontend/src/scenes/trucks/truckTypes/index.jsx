...
const TruckTypes = () => {
    ...
    const handleEdit = (id) => {
        ...
        navigate(
            '/truck-type',
            { state: { loadId: id, loadMode: 'edit', origin: 'truck-types' } }
        );
    }
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const truckTypesClone = [...truckTypes];

            for(let i = 0; i < truckTypesClone.length; i++) {
                const { id } = truckTypesClone[i];
            ...
            if(index !== -1) {
                console.log(truckTypesClone);
                truckTypesClone.splice(index, 1);
                setTruckTypes(truckTypesClone);
            }
    ...
    const handleDuplicate = (id) => {
        ...
        navigate(
            '/truck-type',
            { state: { loadId: id, loadMode: 'duplicate', origin: 'truck-types' } }
        );
    }

    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR TRUCK TYPES?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setTruckTypes([]);
            window.alert(`Storage for truck types reset back to defaults`);
