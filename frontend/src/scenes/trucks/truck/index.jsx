...
import truckClassSchema from "../../../data/schemas/truckClassSchema";
import truckSchema from "../../../data/schemas/truckSchema";
...
const Truck = () => {
    ...
    const [truckEdits, setTruckEdits] = useSessionStorage('truckEdits', []);
    const [newTruckEdit, setNewTruckEdit] = useSessionStorage('newTruckEdit', undefined);
    ...
    const handleSave = (values) => {
        ...
        try {
            ...
            if(typeof(values.id) !== 'undefined') {
                useTruckId = values.id;
                window.alert(`Truck Classes updated for preset # ${useTruckId} with description:\n ${values.description}`);
            } else {
                useTruckId = savedId;
                window.alert(`Truck Classes updated for preset # ${useTruckId} with description:\n ${values.description}`);
            }
            ...
            log(`Error saving truck`);
            console.log(e);
            window.alert(`Error saving truck: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useTruckClassId = handleSave(values);

        if(typeof(useTruckClassId) !== 'undefined') {
            _navigate(useTruckClassId);
        }
    }
    ...
    const updateEditForId = (editId, values) => {
        ...
        try {
            for(let i = 0; i < truckEdits.length; i++) {
                if(truckEdits[i].id === editId) {
                    truckEdits[i] = {...values};
                    foundEdit = true;
                    break;
                }
        ...
        navigate(
            route,
            {
                state: {
                    ...
                    origin: 'truck',  // origin - will return here on save
    ...
    const handleEditClass = (truckType, values) => {
        const {id} = truckTypes.filter(cls => cls.truckType === truckType)[0] || {};
        log(`Editing truck type: ${id}`);
        handleEditSettings(id, values, '/truck-type');
    }

    const handleDuplicateClass = (truckType, values) => {
        const {id} = truckTypes.filter(cls => cls.truckType === truckType)[0] || {};
        log(`Duplicating truck type: ${id}`);
        handleDuplicateSettings(id, values, '/truck-type');
    }

    const handleCreateClass = (values) => {
        log(`Creating new truck type`);
        handleCreateSettings(values, '/truck-type')
    }
    ...
    const truckTypeOptions = truckTypes.map((_option) => {
        return {
            value: _option.id,
            label: _option.truckType,
        }
    });

    return(
        <Box m="20px" minHeight="85vh">
            <Header title="TRUCK" subtitle="Create / edit truck" />
                        ...
                        >
                            ...
                            <FormControl sx={{
                                ...
                                <Select
                                    ...
                                    name="truckType"
                                    value={(truckTypeOptions ? truckTypeOptions.find(option => option.label === values.truckType) : '')}
                                    onChange={(option) => {
                                        setFieldValue( 'truckType', option.label, false  );
                                    }}
                                    onBlur={handleBlur}
                                    styles={customStyles}
                                />
                            ...
                            >
                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleEditClass(values.truckType, values) }}>
                                        EDIT
                                    </Button>
                                </Box>

                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleDuplicateClass(values.truckType, values) }}>
                                        COPY
                                    </Button>
                                </Box>
