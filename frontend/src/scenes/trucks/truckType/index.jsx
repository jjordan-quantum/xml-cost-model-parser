...
import truckTypeSchema from "../../../data/schemas/truckTypeSchema";
...
const TruckType = () => {
    ...
    const [truckTypes, setTruckTypes] = useSessionStorage('truckTypes', []);
    const [truckClasses, setTruckClasses] = useSessionStorage('truckClasses', []);
    const [truckClassEdits, setTruckClassEdits] = useSessionStorage('truckClassEdits', []);
    const [newTruckClassEdit, setNewTruckClassEdit] = useSessionStorage('newTruckClassEdit', undefined);

    const getValuesForId = (id) => {
        try {
            return truckTypes.filter(o => (
                o
    ...
    const handleSave = (values) => {
        ...
        const {description, truckType, truckClass} = values;
        ...
        if(!truckType) {
            log(`truckType undefined - cannot save settings`);
            window.alert(`truckType undefined - cannot save settings`);
            return;
        }

        if(!truckClass) {
            log(`truckClass undefined - cannot save settings`);
            window.alert(`truckClass undefined - cannot save settings`);
            return;
        }

        //const {success, message} = validateGasPriceSettings(values);

        // TODO - validate truck type
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                if(index === -1) {
                    if((truckTypes || []).map((p) => p.truckType.toLowerCase()).includes(truckType.toLowerCase())) {
                        log(`truckType must be unique - already have an entry for:\n\n${truckType}`);
                        window.alert(`truckType must be unique - already have an entry for:\n\n${truckType}`);
                        return;
                    }

                    truckTypes.push({...values});
                } else {
                    truckTypes[index] = {...values};
                }
            } else {
                let maxId = -1;
            ...
            if(typeof(values.id) !== 'undefined') {
                useTruckTypeId = values.id;
                window.alert(`Truck Types updated for preset # ${useTruckTypeId} with description:\n ${values.description} and type ${truckType}`);
            } else {
                useTruckTypeId = savedId;
                window.alert(`Truck Types updated for preset # ${useTruckTypeId} with description:\n ${values.description} and type ${truckType}`);
            }

            return useTruckTypeId;
        } catch(e) {
            log(`Error saving truck type`);
            console.log(e);
            window.alert(`Error saving truck type: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useTruckTypeId = handleSave(values);

        if(typeof(useTruckTypeId) !== 'undefined') {
            _navigate(useTruckTypeId);
        }
    }
    ...
    const updateEditForId = (editId, values) => {
        ...
        try {
            for(let i = 0; i < truckClassEdits.length; i++) {
                if(truckClassEdits[i].id === editId) {
                    truckClassEdits[i] = {...values};
                    foundEdit = true;
                    break;
                }
            }
            ...
            if(!foundEdit) {
                truckClassEdits.push({
                    ...values,
                    id: editId,
                });
            }

            setTruckClassEdits(truckClassEdits);
        } catch(e) {
        ...
        navigate(
            route,
            {
                state: {
                    ...
                    origin: 'truck-type',  // origin - will return here on save
    ...
    const handleEditClass = (truckClass, values) => {
        const {id} = truckClasses.filter(cls => cls.truckClass === truckClass)[0] || {};
        log(`Editing truck class: ${id}`);
        handleEditSettings(id, values, '/truck-class');
    }

    const handleDuplicateClass = (truckClass, values) => {
        const {id} = truckClasses.filter(cls => cls.truckClass === truckClass)[0] || {};
        log(`Duplicating truck class: ${id}`);
        handleDuplicateSettings(id, values, '/truck-class');
    }

    const handleCreateClass = (values) => {
        log(`Creating new truck class`);
        handleCreateSettings(values, '/truck-class')
    }
    ...
    const truckClassOptions = truckClasses.map((_option) => {
        return {
            value: _option.id,
            label: _option.truckClass,
        }
    });

    return(
        <Box m="20px" minHeight="85vh">
            <Header title="TRUCK TYPE" subtitle="Create / edit truck types" />
                        ...
                        >
                            ...
                            <FormControl sx={{
                                ...
                                <Select
                                    ...
                                    name="truckClass"
                                    value={(truckClassOptions ? truckClassOptions.find(option => option.label === values.truckClass) : '')}
                                    onChange={(option) => {
                                        setFieldValue( 'truckClass', option.label, false  );
                                    }}
                                    onBlur={handleBlur}
                                    styles={customStyles}
                                />
                            ...
                            >
                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleEditClass(values.truckClass, values) }}>
                                        EDIT
                                    </Button>
                                </Box>

                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleDuplicateClass(values.truckClass, values) }}>
                                        COPY
                                    </Button>
                                </Box>
                            ...
                            {/*    onBlur={handleBlur}*/}
                            {/*    onChange={handleChange}*/}
                            {/*    value={values.truckClass}*/}
                            {/*    name="truckClass"*/}
                            {/*    error={!!touched.truckClass && !!errors.truckClass}*/}
                            {/*    helperText={touched.truckClass && errors.truckClass}*/}
                            {/*    sx={{ gridColumn: "span 4" }}*/}
                            {/*/>*/}
