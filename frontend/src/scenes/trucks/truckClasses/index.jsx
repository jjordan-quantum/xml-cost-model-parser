...
const TruckClasses = () => {
    ...
    const handleEdit = (id) => {
        ...
        navigate(
            '/truck-class',
            { state: { loadId: id, loadMode: 'edit', origin: 'truck-classes' } }
        );
    }
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const truckClassesClone = [...truckClasses];

            for(let i = 0; i < truckClassesClone.length; i++) {
                const { id } = truckClassesClone[i];
            ...
            if(index !== -1) {
                console.log(truckClassesClone);
                truckClassesClone.splice(index, 1);
                setTruckClasses(truckClassesClone);
            }
    ...
    const handleDuplicate = (id) => {
        ...
        navigate(
            '/truck-class',
            { state: { loadId: id, loadMode: 'duplicate', origin: 'truck-classes' } }
        );
    }

    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR TRUCK CLASSES?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setTruckClasses([]);
            window.alert(`Storage for truck classes reset back to defaults`);
