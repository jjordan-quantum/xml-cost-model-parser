...
const TruckClass = () => {
    const componentName = 'TruckClass';
    ...
    const handleSave = (values) => {
        ...
        const {description, truckClass} = values;
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                for(let i = 0; i < truckClasses.length; i++) {
                    const { id } = truckClasses[i];
                ...
                if(index === -1) {
                    if((truckClasses || []).map((p) => p.truckClass.toLowerCase()).includes(truckClass.toLowerCase())) {
                        log(`truckClass must be unique - already have an entry for:\n\n${truckClass}`);
                        window.alert(`truckClass must be unique - already have an entry for:\n\n${truckClass}`);
                        return;
                    }

                    truckClasses.push({...values});
                } else {
                    truckClasses[index] = {...values};
                }
            } else {
                let maxId = -1;
                ...
                for(const item of truckClasses) {
            ...
            setTruckClasses(truckClasses);
            let useTruckClassId;

            if(typeof(values.id) !== 'undefined') {
                useTruckClassId = values.id;
                window.alert(`Truck Classes updated for preset # ${useTruckClassId} with description:\n ${values.description} and class ${truckClass}`);
            } else {
                useTruckClassId = savedId;
                window.alert(`Truck Classes updated for preset # ${useTruckClassId} with description:\n ${values.description} and class ${truckClass}`);
            }

            return useTruckClassId;
        } catch(e) {
            log(`Error saving truck class`);
            console.log(e);
            window.alert(`Error saving truck class: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    return(
        <Box m="20px" minHeight="85vh">
            <Header title="TRUCK CLASS" subtitle="Create / edit truck classes" />

            <Formik
                onSubmit={handleFormSubmit}
                initialValues={getInitialValues(loadId, loadMode)}
                validationSchema={truckClassSchema}
            >
