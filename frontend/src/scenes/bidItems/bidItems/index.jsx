...
const BidItems = () => {
    ...
    const handleEdit = (id) => {
        ...
        navigate(
            '/bid-item',
            { state: { loadId: id, loadMode: 'edit', origin: 'bid-items' } }
        );
    }
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const bidItemsClone = [...bidItems];

            for(let i = 0; i < bidItemsClone.length; i++) {
                const { id } = bidItemsClone[i];
            ...
            if(index !== -1) {
                console.log(bidItemsClone);
                bidItemsClone.splice(index, 1);
                setBidItems(bidItemsClone);
            }
    ...
    const handleDuplicate = (id) => {
        ...
        navigate(
            '/bid-item',
            { state: { loadId: id, loadMode: 'duplicate', origin: 'bid-items' } }
        );
    }

    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR BID ITEMS?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setBidItems([]);
            window.alert(`Storage for bid items reset back to defaults`);
            navigate('/bid-items',);
        }
    }
