...
const MaterialTypes = () => {
    ...
    const handleEdit = (id) => {
        ...
        navigate(
            '/material-type',
            { state: { loadId: id, loadMode: 'edit', origin: 'material-types' } }
        );
    }
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const materialTypesClone = [...materialTypes];
            ...
            if(index !== -1) {
                ...
                setMaterialTypes(materialTypesClone);
            }
    ...
    const handleDuplicate = (id) => {
        ...
        navigate(
            '/material-type',
            { state: { loadId: id, loadMode: 'duplicate', origin: 'material-types' } }
        );
    }

    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR MATERIAL TYPES?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setMaterialTypes([]);
            window.alert(`Storage for material types reset back to defaults`);
    ...
    return (
        <Box m="20px">
            <Header title="MATERIAL TYPES" subtitle="List of existing presets for Material Types"/>
            <Box display="flex" justifyContent="left" mt="10px" p="10px">
                <Button type="submit" color="secondary" variant="contained" sx={{ ml: 1 }} onClick={() => {
                    navigate(
                        '/material-type',
                        { state: { loadMode: 'new', origin: 'material-types' } }
                    ) ;
                }}>
                    Create New Preset
                </Button>
