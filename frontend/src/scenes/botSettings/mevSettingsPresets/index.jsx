...
const BotMevSettingsPresets = () => {
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const mevSettingsClone = [...botMevSettings];
            ...
            if(index !== -1) {
                ...
                setBotMevSettings(mevSettingsClone);
            }

            navigate(
                '/bot-mev-settings-presets',
            );
        }
    }
    ...
    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR BOT MEV SETTINGS?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setBotMevSettings([]);
            window.alert(`Storage for Bot MEV settings reset back to defaults`);
    ...
    return (
        <Box m="20px">
            <Header title="BOT MEV SETTINGS PRESETS" subtitle="List of existing presets for BOt MEV Settings"/>
...
export default BotMevSettingsPresets;
