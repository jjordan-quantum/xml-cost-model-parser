...
const BotSwapParamsPreset = () => {
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const swapParamsClone = [...botSwapParams];
            ...
            if(index !== -1) {
                ...
                console.log(botSwapParams);
                swapParamsClone.splice(index, 1);
                setBotSwapParams(swapParamsClone);
            } else {
                console.log(`Did not find preset for id`);
            }

            navigate(
                '/bot-swap-params-presets',
            );
        }
    }
    ...
    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR BOT SWAP PARAMETERS?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setBotSwapParams([]);
    ...
    return (
        <Box m="20px">
            <Header title="BOT SWAP PARAMETERS PRESETS" subtitle="List of existing presets for Bot Swap Parameters"/>
...
export default BotSwapParamsPreset;
