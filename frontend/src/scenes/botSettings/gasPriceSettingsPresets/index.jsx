...
const BotGasPriceSettingsPresets = () => {
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const gasPriceSettingsClone = [...botGasPriceSettings];

            for(let i = 0; i < gasPriceSettingsClone.length; i++) {
                const { id } = gasPriceSettingsClone[i];
            ...
            if(index !== -1) {
                console.log(gasPriceSettingsClone);
                gasPriceSettingsClone.splice(index, 1);
                setBotGasPriceSettings(gasPriceSettingsClone);
            }

            navigate(
                '/bot-gas-price-settings-presets',
            );
        }
    }
    ...
    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR BOT GAS PRICE SETTINGS?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setBotGasPriceSettings([]);
            window.alert(`Storage for bot gas price settings reset back to defaults`);
    ...
    const columns = [
        ...
        {
            field: "gasPrice",
            headerName: "Tx gasPrice",
            flex: 1,
        },
        {
            field: "txMaxFeePerGas",
            headerName: "Tx txMaxFeePerGas",
            flex: 1,
        },
        {
            field: "txMaxPriorityFeePerGas",
            headerName: "Tx txMaxPriorityFeePerGas",
            flex: 1,
        },
        // {
        //     field: "frontRun",
        //     headerName: "Frontrunning",
        //     flex: 1,
        // },
        // {
    ...
    return (
        <Box m="20px">
            <Header title="BOT GAS PRICE SETTINGS PRESETS" subtitle="List of existing presets for Bot Gas Price Settings"/>
...
export default BotGasPriceSettingsPresets;
