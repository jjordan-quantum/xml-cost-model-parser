...
import botMevSettingsSchema from "../../../data/schemas/botMevSettingsSchema";
...
const BotMevSettings = () => {
    const componentName = 'BotMevSettings';
    ...
    const [botMevSettingsEdits, setBotMevSettingsEdits] = useSessionStorage('botMevSettingsEdits', []);
    const [newBotMevSettingsEdit, setNewBotMevSettingsEdit] = useSessionStorage('newBotMevSettingsEdit', undefined);
    ...
    const getEditsForId = (id) => {
        try {
            return botMevSettingsEdits.filter(o => (
                o
                && typeof(o.id) !== 'undefined'
                && (o.id === id)
            ))[0];
    ...
    const updateEditForId = (editId, values) => {
        ...
        try {
            for(let i = 0; i < botMevSettingsEdits.length; i++) {
                if(botMevSettingsEdits[i].id === editId) {
                    botMevSettingsEdits[i] = {...values};
                    foundEdit = true;
                    break;
                }
            }
            ...
            if(!foundEdit) {
                botMevSettingsEdits.push({...values});
            }

            setBotMevSettingsEdits(botMevSettingsEdits);
        } catch(e) {
    ...
    const saveEdit = (values) => {
        ...
        if(typeof(values.id) !== 'undefined') {
            ...
            setNewBotMevSettingsEdit(values);
        }
    }
        ...
        try {
            if(origin && origin === 'executor') {
                ...
                if(typeof(_loadId) === 'undefined') {
                    return {
                        ...(newBotMevSettingsEdit || initialValues),
                ...
                if(typeof(_loadId) === 'undefined') {
                    return {...(newBotMevSettingsEdit || initialValues)};
                }
    ...
    const _navigate = (_id) => {
        ...
        if(dest) {
            ...
            navigate('/bot-mev-settings-presets');
        }
    }
    ...
    const handleSave = (values) => {
        ...
        try {
            ...
            if(typeof(values.id) !== 'undefined') {
                ...
                for(let i = 0; i < botMevSettings.length; i++) {
                    const { id } = botMevSettings[i];
                ...
                if(index === -1) {
                    // did not find
                    if(!isDescriptionUnique(description, botMevSettings || [])) {
                    ...
                    botMevSettings.push({ ...values, mevContractAddress, mevDeployerPrivateKey });
                } else {
                    botMevSettings[index] = { ...values, mevContractAddress, mevDeployerPrivateKey };
                }
            } else {
                let maxId = -1;
                ...
                for(const item of botMevSettings) {
            ...
            setBotMevSettings(botMevSettings);
            let useMevSettingsId;

            if(typeof(values.id) !== 'undefined') {
                useMevSettingsId = values.id;

                window.alert(`Bot MEV Settings updated for preset # ${values.id} with description:\n ${values.description} 
                ...
                window.alert(`Bot MEV Settings saved as preset # ${savedId} with description: \n ${values.description} 
                    ${success ? '\nSaved settings: ' + stringify(values) : ('\nWARNING: Settings failed validation: ' + message + '\n\nThey will still be saved, but will not pass validation if sent to sniper app')}
                `);
            }
    ...
    return(
        <Box m="20px" minHeight="85vh">
            <Header title="BOT MEV SETTINGS" subtitle="Create / edit Bot MEV settings preset" />

            <Formik
                onSubmit={handleFormSubmit}
                initialValues={getInitialValues(loadId, loadMode)}
                validationSchema={botMevSettingsSchema}
            >
