...
const BotSellTriggerConditionsPresets = () => {
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const sellTriggerConditionsClone = [...botSellTriggerConditions];
            ...
            if(index !== -1) {
                ...
                setBotSellTriggerConditions(sellTriggerConditionsClone);
            }

            navigate(
                '/bot-sell-trigger-conditions-presets',
            );
        }
    }
    ...
    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR BOT SELL TRIGGER CONDITIONS?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setBotSellTriggerConditions([]);
            window.alert(`Storage for bot sell trigger conditions reset back to defaults`);
    ...
    const columns = [
        ...
        {
            field: "dumpStaleTokens",
            headerName: "Dump Stale Tokens",
            flex: 1,
        },
        {
            field: "sellAtTargetRoi",
            headerName: "Sell At Target ROI",
            flex: 1,
        },
        {
            field: "fee",
    ...
    return (
        <Box m="20px">
            <Header title="BOT SELL TRIGGER CONDITIONS PRESETS" subtitle="List of existing presets for Bot Sell Trigger Conditions"/>
...
export default BotSellTriggerConditionsPresets;
