...
const VehicleType = () => {
    const componentName = 'VehicleType';
    ...
    const [vehicleTypeEdits, setVehicleTypeEdits] = useSessionStorage('vehicleTypeEdits', []);
    const [newVehicleTypeEdit, setNewVehicleTypeEdit] = useSessionStorage('newVehicleTypeEdit', undefined);

    const getValuesForId = (id) => {
        try {
            return vehicleTypes.filter(o => (
                o
    ...
    const handleSave = (values) => {
        ...
        const {description, vehicleType, vehicleClass} = values;
        ...
        if(!vehicleType) {
            log(`vehicleType undefined - cannot save settings`);
            window.alert(`vehicleType undefined - cannot save settings`);
            return;
        }
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                if(index === -1) {
                    if((vehicleTypes || []).map((p) => p.vehicleType.toLowerCase()).includes(vehicleType.toLowerCase())) {
                        log(`vehicleType must be unique - already have an entry for:\n\n${vehicleType}`);
                        window.alert(`vehicleType must be unique - already have an entry for:\n\n${vehicleType}`);
                        return;
                    }

                    vehicleTypes.push({...values});
                } else {
                    vehicleTypes[index] = {...values};
                }
            } else {
                let maxId = -1;
            ...
            let useVehicleTypeId;

            if(typeof(values.id) !== 'undefined') {
                useVehicleTypeId = values.id;
                window.alert(`Vehicle Types updated for preset # ${useVehicleTypeId} with description:\n ${values.description} and type ${vehicleType}`);
            } else {
                useVehicleTypeId = savedId;
                window.alert(`Vehicle Types updated for preset # ${useVehicleTypeId} with description:\n ${values.description} and type ${vehicleType}`);
            }

            return useVehicleTypeId;
        } catch(e) {
            log(`Error saving vehicle type`);
            console.log(e);
            window.alert(`Error saving vehicle type: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useVehicleTypeId = handleSave(values);

        if(typeof(useVehicleTypeId) !== 'undefined') {
            _navigate(useVehicleTypeId);
        }
    }
    ...
    const updateEditForId = (editId, values) => {
        ...
        try {
            for(let i = 0; i < vehicleTypeEdits.length; i++) {
                if(vehicleTypeEdits[i].id === editId) {
                    vehicleTypeEdits[i] = {...values};
                    foundEdit = true;
                    break;
                }
            }
            ...
            if(!foundEdit) {
                vehicleTypeEdits.push({
                    ...values,
                    id: editId,
                });
            }

            setVehicleTypeEdits(vehicleTypeEdits);
        } catch(e) {
        ...
        navigate(
            route,
            {
                state: {
                    ...
                    origin: 'vehicle-type',  // origin - will return here on save
    ...
    const handleEditClass = (vehicleClass, values) => {
        const {id} = vehicleClasses.filter(cls => cls.vehicleClass === vehicleClass)[0] || {};
        log(`Editing vehicle class: ${id}`);
        handleEditSettings(id, values, '/vehicle-class');
    }

    const handleDuplicateClass = (vehicleClass, values) => {
        const {id} = vehicleClasses.filter(cls => cls.vehicleClass === vehicleClass)[0] || {};
        log(`Duplicating vehicle class: ${id}`);
        handleDuplicateSettings(id, values, '/vehicle-class');
    }

    const handleCreateClass = (values) => {
        log(`Creating new vehicle class`);
        handleCreateSettings(values, '/vehicle-class')
    }
                        ...
                        >
                            ...
                            <FormControl sx={{
                                ...
                                <Select
                                    ...
                                    name="vehicleClass"
                                    value={(vehicleClassOptions ? vehicleClassOptions.find(option => option.label === values.vehicleClass) : '')}
                                    onChange={(option) => {
                                        setFieldValue( 'vehicleClass', option.label, false  );
                                    }}
                                    onBlur={handleBlur}
                                    styles={customStyles}
                                />
                            ...
                            >
                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleEditClass(values.vehicleClass, values) }}>
                                        EDIT
                                    </Button>
                                </Box>

                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleDuplicateClass(values.vehicleClass, values) }}>
                                        COPY
                                    </Button>
                                </Box>
