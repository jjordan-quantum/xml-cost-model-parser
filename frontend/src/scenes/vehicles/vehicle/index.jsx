...
import vehicleSchema from "../../../data/schemas/vehicleSchema";
...
const Vehicle = () => {
    ...
    const [vehicleEdits, setVehicleEdits] = useSessionStorage('vehicleEdits', []);
    const [newVehicleEdit, setNewVehicleEdit] = useSessionStorage('newVehicleEdit', undefined);
    ...
    const handleSave = (values) => {
        ...
        try {
            ...
            if(typeof(values.id) !== 'undefined') {
                useVehicleId = values.id;
                window.alert(`Vehicles updated for preset # ${useVehicleId} with description:\n ${values.description}`);
            } else {
                useVehicleId = savedId;
                window.alert(`Vehicles updated for preset # ${useVehicleId} with description:\n ${values.description}`);
            }

            return useVehicleId;
        } catch(e) {
            log(`Error saving vehicle`);
            console.log(e);
            window.alert(`Error saving vehicle: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useVehicleId = handleSave(values);

        if(typeof(useVehicleId) !== 'undefined') {
            _navigate(useVehicleId);
        }
    }
    ...
    const updateEditForId = (editId, values) => {
        ...
        try {
            for(let i = 0; i < vehicleEdits.length; i++) {
                if(vehicleEdits[i].id === editId) {
                    vehicleEdits[i] = {...values};
                    foundEdit = true;
                    break;
                }
            }
        ...
        navigate(
            route,
            {
                state: {
                    ...
                    origin: 'vehicle',  // origin - will return here on save
    ...
    const handleEditClass = (vehicleType, values) => {
        const {id} = vehicleTypes.filter(cls => cls.vehicleType === vehicleType)[0] || {};
        log(`Editing vehicle type: ${id}`);
        handleEditSettings(id, values, '/vehicle-type');
    }

    const handleDuplicateClass = (vehicleType, values) => {
        const {id} = vehicleTypes.filter(cls => cls.vehicleType === vehicleType)[0] || {};
        log(`Duplicating vehicle type: ${id}`);
        handleDuplicateSettings(id, values, '/vehicle-type');
    }

    const handleCreateClass = (values) => {
        log(`Creating new vehicle type`);
        handleCreateSettings(values, '/vehicle-type')
    }
    ...
    const vehicleTypeOptions = vehicleTypes.map((_option) => {
        return {
            value: _option.id,
            label: _option.vehicleType,
        }
    });

    return(
        <Box m="20px" minHeight="85vh">
            <Header title="VEHICLE" subtitle="Create / edit vehicles" />
                        ...
                        >
                            ...
                            <FormControl sx={{
                                ...
                                <Select
                                    ...
                                    name="vehicleType"
                                    value={(vehicleTypeOptions ? vehicleTypeOptions.find(option => option.label === values.vehicleType) : '')}
                                    onChange={(option) => {
                                        setFieldValue( 'vehicleType', option.label, false  );
                                    }}
                                    onBlur={handleBlur}
                                    styles={customStyles}
                                />
                            ...
                            >
                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleEditClass(values.vehicleType, values) }}>
                                        EDIT
                                    </Button>
                                </Box>

                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleDuplicateClass(values.vehicleType, values) }}>
                                        COPY
                                    </Button>
                                </Box>
