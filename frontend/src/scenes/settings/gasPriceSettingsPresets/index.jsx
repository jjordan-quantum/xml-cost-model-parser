...
const GasPriceSettingsPresets = () => {
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const gasPriceSettingsClone = [...gasPriceSettings];
            ...
            if(index !== -1) {
                ...
                setGasPriceSettings(gasPriceSettingsClone);
            }

            navigate(
                '/gas-price-settings-presets',
            );
        }
    }
    ...
    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR GAS PRICE SETTINGS?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setGasPriceSettings([]);
            window.alert(`Storage for gas price settings reset back to defaults`);
            navigate('/gas-price-settings-presets',);
        }
    }
    ...
    return (
        <Box m="20px">
            <Header title="GAS PRICE SETTINGS PRESETS" subtitle="List of existing presets for Gas Price Settings"/>
...
export default GasPriceSettingsPresets;
