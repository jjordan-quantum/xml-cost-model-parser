...
const GasPriceSettings = () => {
    const componentName = 'GasPriceSettings';
    ...
    const handleSave = (values) => {
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                for(let i = 0; i < gasPriceSettings.length; i++) {
                    const { id } = gasPriceSettings[i];
                ...
                if(index === -1) {
                    // did not find
                    if(!isDescriptionUnique(description, gasPriceSettings || [])) {
                    ...
                    gasPriceSettings.push({...values});
                } else {
                    gasPriceSettings[index] = {...values};
                }
            } else {
                let maxId = -1;
                ...
                for(const item of gasPriceSettings) {
            ...
            setGasPriceSettings(gasPriceSettings);
    ...
    return(
        <Box m="20px" minHeight="85vh">
            <Header title="GAS PRICE SETTINGS" subtitle="Create / edit gas price settings preset" />

            <Formik
                onSubmit={handleFormSubmit}
                initialValues={getInitialValues(loadId, loadMode)}
                validationSchema={gasPriceSettingsSchema}
            >
...
export default GasPriceSettings;
