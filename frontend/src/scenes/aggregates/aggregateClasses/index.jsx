...
const AggregateClasses = () => {
    ...
    const handleEdit = (id) => {
        ...
        navigate(
            '/aggregate-class',
            { state: { loadId: id, loadMode: 'edit', origin: 'aggregate-classes' } }
        );
    }
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const aggregateClassesClone = [...aggregateClasses];

            for(let i = 0; i < aggregateClassesClone.length; i++) {
                const { id } = aggregateClassesClone[i];
            ...
            if(index !== -1) {
                console.log(aggregateClassesClone);
                aggregateClassesClone.splice(index, 1);
                setAggregateClasses(aggregateClassesClone);
            }
    ...
    const handleDuplicate = (id) => {
        ...
        navigate(
            '/aggregate-class',
            { state: { loadId: id, loadMode: 'duplicate', origin: 'aggregate-classes' } }
        );
    }

    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR AGGREGATE CLASSES?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setAggregateClasses([]);
            window.alert(`Storage for aggregate classes reset back to defaults`);
    ...
    return (
        <Box m="20px">
            <Header title="AGGREGATE CLASSES" subtitle="List of existing presets for Aggregate Classes"/>
            <Box display="flex" justifyContent="left" mt="10px" p="10px">
                <Button type="submit" color="secondary" variant="contained" sx={{ ml: 1 }} onClick={() => {
                    navigate(
                        '/aggregate-class',
                        { state: { loadMode: 'new', origin: 'aggregate-classes' } }
                    ) ;
                }}>
                    Create New Preset
                </Button>
