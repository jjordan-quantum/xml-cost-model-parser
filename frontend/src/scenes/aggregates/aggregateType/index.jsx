...
const AggregateType = () => {
    const componentName = 'AggregateType';
    ...
    const [aggregateClassEdits, setAggregateClassEdits] = useSessionStorage('aggregateClassEdits', []);
    const [newAggregateClassEdit, setNewAggregateClassEdit] = useSessionStorage('newAggregateClassEdit', undefined);

    const getValuesForId = (id) => {
        try {
            return aggregateTypes.filter(o => (
                o
    ...
    const handleSave = (values) => {
        ...
        const {description, aggregateType, aggregateClass} = values;
        ...
        if(!aggregateType) {
            log(`aggregateType undefined - cannot save settings`);
            window.alert(`aggregateType undefined - cannot save settings`);
            return;
        }
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                for(let i = 0; i < aggregateTypes.length; i++) {
                    const { id } = aggregateTypes[i];
                ...
                if(index === -1) {
                    if((aggregateTypes || []).map((p) => p.aggregateType.toLowerCase()).includes(aggregateType.toLowerCase())) {
                        log(`aggregateType must be unique - already have an entry for:\n\n${aggregateType}`);
                        window.alert(`aggregateType must be unique - already have an entry for:\n\n${aggregateType}`);
                        return;
                    }

                    aggregateTypes.push({...values});
                } else {
                    aggregateTypes[index] = {...values};
                }
            } else {
                let maxId = -1;
                ...
                for(const item of aggregateTypes) {
            ...
            let useAggregateTypeId;

            if(typeof(values.id) !== 'undefined') {
                useAggregateTypeId = values.id;
                window.alert(`Aggregate Types updated for preset # ${useAggregateTypeId} with description:\n ${values.description} and type ${aggregateType}`);
            } else {
                useAggregateTypeId = savedId;
                window.alert(`Aggregate Types updated for preset # ${useAggregateTypeId} with description:\n ${values.description} and type ${aggregateType}`);
            }

            return useAggregateTypeId;
        } catch(e) {
            log(`Error saving aggregate type`);
            console.log(e);
            window.alert(`Error saving aggregate type: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useAggregateTypeId = handleSave(values);

        if(typeof(useAggregateTypeId) !== 'undefined') {
            _navigate(useAggregateTypeId);
        }
    }
    ...
    const aggregateClassOptions = aggregateClasses.map((_option) => {
        return {
            value: _option.id,
            label: _option.aggregateClass,
        }
    });

    return(
        <Box m="20px" minHeight="85vh">
            <Header title="AGGREGATE TYPE" subtitle="Create / edit aggregate types" />

            <Formik
                onSubmit={handleFormSubmit}
                initialValues={getInitialValues(loadId, loadMode)}
                validationSchema={aggregateTypeSchema}
            >
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        >
                            ...
                            <TextField
                                ...
                                name="aggregateType"
                                error={!!touched.aggregateType && !!errors.aggregateType}
                                helperText={touched.aggregateType && errors.aggregateType}
                                sx={{ gridColumn: "span 4" }}
                            />
                            ...
                            <FormControl sx={{
                                ...
                                <Select
                                    options={aggregateClassOptions}
                                    name="aggregateClass"
                                    value={(aggregateClassOptions ? aggregateClassOptions.find(option => option.label === values.aggregateClass) : '')}
                                    onChange={(option) => {
                                        setFieldValue( 'aggregateClass', option.label, false  );
                                    }}
                                    onBlur={handleBlur}
                                    styles={customStyles}
                                />
                            ...
                            >
                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleEditClass(values.aggregateClass, values) }}>
                                        EDIT
                                    </Button>
                                </Box>

                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleDuplicateClass(values.aggregateClass, values) }}>
                                        COPY
                                    </Button>
                                </Box>
