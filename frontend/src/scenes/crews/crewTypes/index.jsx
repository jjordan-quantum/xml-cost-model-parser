...
const CrewTypes = () => {
    ...
    const handleEdit = (id) => {
        ...
        navigate(
            '/crew-type',
            { state: { loadId: id, loadMode: 'edit', origin: 'crew-types' } }
        );
    }
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const crewTypesClone = [...crewTypes];

            for(let i = 0; i < crewTypesClone.length; i++) {
                const { id } = crewTypesClone[i];
            ...
            if(index !== -1) {
                console.log(crewTypesClone);
                crewTypesClone.splice(index, 1);
                setCrewTypes(crewTypesClone);
            }
    ...
    const handleDuplicate = (id) => {
        ...
        navigate(
            '/crew-type',
            { state: { loadId: id, loadMode: 'duplicate', origin: 'crew-types' } }
        );
    }

    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR CREW TYPES?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setCrewTypes([]);
            window.alert(`Storage for crew types reset back to defaults`);
            navigate('/crew-types',);
        }
    }
