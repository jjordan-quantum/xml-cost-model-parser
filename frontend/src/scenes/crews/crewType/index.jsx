...
import crewTypeSchema from "../../../data/schemas/crewTypeSchema";
...
const CrewType = () => {
    ...
    const handleSave = (values) => {
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                if(index === -1) {
                    crewTypes.push({...values});
                } else {
                    crewTypes[index] = {...values};
                }
            } else {
                let maxId = -1;

                for(const item of crewTypes) {
            ...
            setCrewTypes(crewTypes);
            let useCrewTypeId;

            if(typeof(values.id) !== 'undefined') {
                useCrewTypeId = values.id;
                window.alert(`Crew Types updated for preset # ${useCrewTypeId} with description:\n ${values.description}`);
            } else {
                useCrewTypeId = savedId;
                window.alert(`Crews Types updated for preset # ${useCrewTypeId} with description:\n ${values.description}`);
            }

            return useCrewTypeId;
        } catch(e) {
            log(`Error saving crew type`);
            console.log(e);
            window.alert(`Error saving crew type: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useCrewTypeId = handleSave(values);

        if(typeof(useCrewTypeId) !== 'undefined') {
            _navigate(useCrewTypeId);
        }
    }
    ...
    return(
        <Box m="20px" minHeight="85vh">
            <Header title="CREW TYPE" subtitle="Create / edit crew type" />

            <Formik
                onSubmit={handleFormSubmit}
                initialValues={getInitialValues(loadId, loadMode)}
                validationSchema={crewTypeSchema}
            >
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        >
                            ...
                            <TextField
                                ...
                                onBlur={handleBlur}
                                onChange={handleChange}
                                value={values.crewType}
                                name="crewType"
                                error={!!touched.crewType && !!errors.crewType}
                                helperText={touched.crewType && errors.crewType}
                                sx={{ gridColumn: "span 4" }}
                            />
