...
const IncidentalCosts = () => {
    ...
    const handleEdit = (id) => {
        ...
        navigate(
            '/incidental-cost',
            { state: { loadId: id, loadMode: 'edit', origin: 'incidental-costs' } }
        );
    }
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const incidentalCostsClone = [...incidentalCosts];

            for(let i = 0; i < incidentalCostsClone.length; i++) {
                const { id } = incidentalCostsClone[i];
            ...
            if(index !== -1) {
                console.log(incidentalCostsClone);
                incidentalCostsClone.splice(index, 1);
                setIncidentalCosts(incidentalCostsClone);
            }
    ...
    const handleDuplicate = (id) => {
        ...
        navigate(
            '/incidental-cost',
            { state: { loadId: id, loadMode: 'duplicate', origin: 'incidental-costs' } }
        );
    }

    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR INCIDENTAL COSTS?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setIncidentalCosts([]);
            window.alert(`Storage for incidental costs reset back to defaults`);
    ...
    const columns = [
        ...
        {
            field: "incidentalCost",
            headerName: "Incidental Cost",
            flex: 1,
        },
        {
            field: "description",
    ...
    return (
        <Box m="20px">
            <Header title="INCIDENTAL COSTS" subtitle="List of existing presets for Incidental Costs"/>
            <Box display="flex" justifyContent="left" mt="10px" p="10px">
                <Button type="submit" color="secondary" variant="contained" sx={{ ml: 1 }} onClick={() => {
                    navigate(
                        '/incidental-cost',
                        { state: { loadMode: 'new', origin: 'incidental-costs' } }
                    ) ;
                }}>
                    Create New Preset
                </Button>
