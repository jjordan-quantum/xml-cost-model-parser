...
const IncidentalCostTypes = () => {
    ...
    const handleEdit = (id) => {
        ...
        navigate(
            ...
            { state: { loadId: id, loadMode: 'edit', origin: 'incidental-cost-types' } }
        );
    }
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const incidentalCostTypesClone = [...incidentalCostTypes];

            for(let i = 0; i < incidentalCostTypesClone.length; i++) {
                const { id } = incidentalCostTypesClone[i];
            ...
            if(index !== -1) {
                console.log(incidentalCostTypesClone);
                incidentalCostTypesClone.splice(index, 1);
                setIncidentalCostTypes(incidentalCostTypesClone);
            }

            navigate(
                '/incidental-cost-types',
            );
        }
    }
    ...
    const handleDuplicate = (id) => {
        ...
        navigate(
            '/incidental-cost-type',
            { state: { loadId: id, loadMode: 'duplicate', origin: 'incidental-cost-types' } }
        );
    }

    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR INCIDENTAL COST TYPES?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setIncidentalCostTypes([]);
            window.alert(`Storage for incidental cost types reset back to defaults`);
    ...
    return (
        <Box m="20px">
            <Header title="INCIDENTAL COST TYPES" subtitle="List of existing presets for Incidental Cost Types"/>
            <Box display="flex" justifyContent="left" mt="10px" p="10px">
                <Button type="submit" color="secondary" variant="contained" sx={{ ml: 1 }} onClick={() => {
                    navigate(
                        '/incidental-cost-type',
                        { state: { loadMode: 'new', origin: 'incidental-cost-types' } }
                    ) ;
                }}>
                    Create New Preset
                </Button>
...
export default IncidentalCostTypes;
