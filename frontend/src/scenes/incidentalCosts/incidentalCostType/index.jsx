...
import incidentalCostTypeSchema from "../../../data/schemas/incidentalCostTypeSchema";
...
const IncidentalCostType = () => {
    const componentName = 'IncidentalCostType';
    ...
    const getValuesForId = (id) => {
        try {
            return incidentalCostTypes.filter(o => (
                o
    ...
    const _navigate = (_id) => {
        if(origin) {
            ...
            navigate('/incidental-cost-types');
        }
    }
    ...
    const handleSave = (values) => {
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                for(let i = 0; i < incidentalCostTypes.length; i++) {
                    const { id } = incidentalCostTypes[i];
                ...
                if(index === -1) {
                    incidentalCostTypes.push({...values});
                } else {
                    incidentalCostTypes[index] = {...values};
                }
            } else {
                let maxId = -1;

                for(const item of incidentalCostTypes) {
            ...
            setIncidentalCostTypes(incidentalCostTypes);
            let useIncidentalCostTypeId;

            if(typeof(values.id) !== 'undefined') {
                useIncidentalCostTypeId = values.id;
                window.alert(`Incidental Cost Types updated for preset # ${useIncidentalCostTypeId} with description:\n ${values.description}`);
            } else {
                useIncidentalCostTypeId = savedId;
                window.alert(`Incidental Cost Types updated for preset # ${useIncidentalCostTypeId} with description:\n ${values.description}`);
            }

            return useIncidentalCostTypeId;
        } catch(e) {
            log(`Error saving incidental cost type`);
            console.log(e);
            window.alert(`Error saving incidental cost type: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useIncidentalCostTypeId = handleSave(values);

        if(typeof(useIncidentalCostTypeId) !== 'undefined') {
            _navigate(useIncidentalCostTypeId);
        }
    }
    ...
    return(
        <Box m="20px" minHeight="85vh">
            <Header title="INCIDENTAL COST TYPE" subtitle="Create / edit incidental cost types" />

            <Formik
                onSubmit={handleFormSubmit}
                initialValues={getInitialValues(loadId, loadMode)}
                validationSchema={incidentalCostTypeSchema}
            >
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        >
                            ...
                            <TextField
                                ...
                                variant="filled"
                                type="text"
                                label="Incidental Cost Type"
                                onBlur={handleBlur}
                                onChange={handleChange}
                                value={values.incidentalCostType}
                                name="incidentalCostType"
                                error={!!touched.incidentalCostType && !!errors.incidentalCostType}
                                helperText={touched.incidentalCostType && errors.incidentalCostType}
                                sx={{ gridColumn: "span 4" }}
                            />
...
export default IncidentalCostType;
