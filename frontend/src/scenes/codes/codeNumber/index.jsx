...
import codeNumberSchema from "../../../data/schemas/codeNumberSchema";
...
const CodeNumber = () => {
    const componentName = 'CodeNumber';
    ...
    const [codeNumbersEdits, setCodeNumbersEdits] = useSessionStorage('codeNumbersEdits', []);
    const [newCodeNumberEdit, setNewCodeNumberEdit] = useSessionStorage('newCodeNumberEdit', undefined);

    const getValuesForId = (id) => {
        try {
            return codeNumbers.filter(o => (
                o
    ...
    const handleSave = (values) => {
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                if(index === -1) {
                    codeNumbers.push({...values});
                } else {
                    codeNumbers[index] = {...values};
                }
            } else {
                let maxId = -1;

                for(const item of codeNumbers) {
            ...
            setCodeNumbers(codeNumbers);
            let useCodeNumberId;

            if(typeof(values.id) !== 'undefined') {
                useCodeNumberId = values.id;
                window.alert(`Code Numbers updated for preset # ${useCodeNumberId} with description:\n ${values.description}`);
            } else {
                useCodeNumberId = savedId;
                window.alert(`Code Numbers updated for preset # ${useCodeNumberId} with description:\n ${values.description}`);
            }

            return useCodeNumberId;
        } catch(e) {
            log(`Error saving code sub categories`);
            console.log(e);
            window.alert(`Error saving code sub categories: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useCodeNumberId = handleSave(values);

        if(typeof(useCodeNumberId) !== 'undefined') {
            _navigate(useCodeNumberId);
        }
    }
    ...
    const updateEditForId = (editId, values) => {
        ...
        try {
            for(let i = 0; i < codeNumbersEdits.length; i++) {
                if(codeNumbersEdits[i].id === editId) {
                    codeNumbersEdits[i] = {...values};
                    foundEdit = true;
                    break;
                }
            }
            ...
            if(!foundEdit) {
                codeNumbersEdits.push({
                    ...values,
                    id: editId,
                });
            }

            setCodeNumbersEdits(codeNumbersEdits);
        } catch(e) {
        ...
        navigate(
            route,
            {
                state: {
                    ...
                    origin: 'code-number',  // origin - will return here on save
    ...
    const handleEditClass = (codeSubCategory, values) => {
        const {id} = codeSubCategories.filter(cls => cls.codeSubCategory === codeSubCategory)[0] || {};
        log(`Editing code sub category: ${id}`);
        handleEditSettings(id, values, '/code-sub-category');
    }

    const handleDuplicateClass = (codeSubCategory, values) => {
        const {id} = codeSubCategories.filter(cls => cls.codeSubCategory === codeSubCategory)[0] || {};
        log(`Duplicating code sub category: ${id}`);
        handleDuplicateSettings(id, values, '/code-sub-category');
    }

    const handleCreateClass = (values) => {
        log(`Creating new code sub category`);
        handleCreateSettings(values, '/code-sub-category')
    }
    ...
    const codeSubCategoryOptions = codeSubCategories.map((_option) => {
        return {
            value: _option.id,
            label: `${_option.codeCategoryNumber}-${_option.codeSubCategoryNumber} - ${_option.codeSubCategory}`,
    ...
    return(
        <Box m="20px" minHeight="85vh">
            <Header title="CODE SUB NUMBERS" subtitle="Create / edit code sub numbers" />
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        >
                            ...
                            <TextField
                                ...
                                name="codeNumber"
                                error={!!touched.codeNumber && !!errors.codeNumber}
                            ...
                            <TextField
                                ...
                                name="codeName"
                                error={!!touched.codeName && !!errors.codeName}
                            ...
                            <FormControl sx={{
                                ...
                                <Select
                                    options={codeSubCategoryOptions}
                                    name="codeSubCategory"
                                    value={(codeSubCategoryOptions ? codeSubCategoryOptions.find(option => option.codeSubCategory === values.codeSubCategory) : '')}
                                    onChange={(option) => {
                            ...
                            >
                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleEditClass(values.codeSubCategory, values) }}>
                                        EDIT
                                    </Button>
                                </Box>

                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleDuplicateClass(values.codeSubCategory, values) }}>
                                        COPY
                                    </Button>
                                </Box>
