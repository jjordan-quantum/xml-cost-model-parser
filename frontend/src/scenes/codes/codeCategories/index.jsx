...
const CodeCategories = () => {
    ...
    const handleEdit = (id) => {
        ...
        navigate(
            '/code-category',
            { state: { loadId: id, loadMode: 'edit', origin: 'code-categories' } }
        );
    }
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const codeCategoriesClone = [...codeCategories];

            for(let i = 0; i < codeCategoriesClone.length; i++) {
                const { id } = codeCategoriesClone[i];
            ...
            if(index !== -1) {
                console.log(codeCategoriesClone);
                codeCategoriesClone.splice(index, 1);
                setCodeCategories(codeCategoriesClone);
            }
    ...
    const handleDuplicate = (id) => {
        ...
        navigate(
            '/code-category',
            { state: { loadId: id, loadMode: 'duplicate', origin: 'code-categories' } }
        );
    }

    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR DIVISIONS?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setCodeCategories([]);
            window.alert(`Storage for divisions reset back to defaults`);
