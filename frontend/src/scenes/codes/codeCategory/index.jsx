...
import codeCategorySchema from "../../../data/schemas/codeCategorySchema";
...
const CodeCategory = () => {
    const componentName = 'CodeCategory';
    ...
    const getValuesForId = (id) => {
        try {
            return codeCategories.filter(o => (
                o
    ...
    const handleSave = (values) => {
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                if(index === -1) {
                    codeCategories.push({...values});
                } else {
                    codeCategories[index] = {...values};
                }
            } else {
                let maxId = -1;

                for(const item of codeCategories) {
            ...
            setCodeCategories(codeCategories);
            let useCodeCategoryId;

            if(typeof(values.id) !== 'undefined') {
                useCodeCategoryId = values.id;
                window.alert(`Divisions updated for preset # ${useCodeCategoryId} with description:\n ${values.description}`);
            } else {
                useCodeCategoryId = savedId;
                window.alert(`Divisions updated for preset # ${useCodeCategoryId} with description:\n ${values.description}`);
            }

            return useCodeCategoryId;
        } catch(e) {
            log(`Error saving category code`);
            console.log(e);
            window.alert(`Error saving category code: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    return(
        <Box m="20px" minHeight="85vh">
            <Header title="DIVISION" subtitle="Create / edit code divisions" />

            <Formik
                onSubmit={handleFormSubmit}
                initialValues={getInitialValues(loadId, loadMode)}
                validationSchema={codeCategorySchema}
            >
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        >
                            ...
                            <TextField
                                ...
                                onBlur={handleBlur}
                                onChange={handleChange}
                                value={values.codeCategoryNumber}
                                name="codeCategoryNumber"
                                error={!!touched.codeCategoryNumber && !!errors.codeCategoryNumber}
                                helperText={touched.codeCategoryNumber && errors.codeCategoryNumber}
                                sx={{ gridColumn: "span 4" }}
                            />
                            ...
                            <TextField
                                ...
                                onBlur={handleBlur}
                                onChange={handleChange}
                                value={values.codeCategory}
                                name="codeCategory"
                                error={!!touched.codeCategory && !!errors.codeCategory}
                                helperText={touched.codeCategory && errors.codeCategory}
                                sx={{ gridColumn: "span 4" }}
                            />
