...
import codeSubCategorySchema from "../../../data/schemas/codeSubCategorySchema";
...
const CodeSubCategory = () => {
    const componentName = 'CodeSubCategory';
    ...
    const [codeCategories, setCodeCategories] = useSessionStorage('codeCategories', []);
    const [codeSubCategoriesEdits, setCodeSubCategoriesEdits] = useSessionStorage('codeSubCategoriesEdits', []);
    const [newCodeSubCategoryEdit, setNewCodeSubCategoryEdit] = useSessionStorage('newCodeSubCategoryEdit', undefined);

    const getValuesForId = (id) => {
        try {
            return codeSubCategories.filter(o => (
                o
    ...
    const handleSave = (values) => {
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                for(let i = 0; i < codeSubCategories.length; i++) {
                    const { id } = codeSubCategories[i];
                ...
                if(index === -1) {
                    codeSubCategories.push({...values});
                } else {
                    codeSubCategories[index] = {...values};
                }
            } else {
                let maxId = -1;

                for(const item of codeSubCategories) {
            ...
            setCodeSubCategories(codeSubCategories);
            let useCodeSubCategoryId;

            if(typeof(values.id) !== 'undefined') {
                useCodeSubCategoryId = values.id;
                window.alert(`Code Categories updated for preset # ${useCodeSubCategoryId} with description:\n ${values.description}`);
            } else {
                useCodeSubCategoryId = savedId;
                window.alert(`Code Categories updated for preset # ${useCodeSubCategoryId} with description:\n ${values.description}`);
            }

            return useCodeSubCategoryId;
        } catch(e) {
            log(`Error saving code categories`);
            console.log(e);
            window.alert(`Error saving code categories: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useRentalId = handleSave(values);

        if(typeof(useRentalId) !== 'undefined') {
            _navigate(useRentalId);
        }
    }
    ...
    const updateEditForId = (editId, values) => {
        ...
        try {
            for(let i = 0; i < codeSubCategoriesEdits.length; i++) {
                if(codeSubCategoriesEdits[i].id === editId) {
                    codeSubCategoriesEdits[i] = {...values};
                    foundEdit = true;
                    break;
                }
            }
            ...
            if(!foundEdit) {
                codeSubCategoriesEdits.push({
                    ...values,
                    id: editId,
                });
            }

            setCodeSubCategoriesEdits(codeSubCategoriesEdits);
        } catch(e) {
    ...
    const saveEdit = (values) => {
        ...
        if(typeof(values.id) !== 'undefined') {
            ...
            setNewCodeSubCategoryEdit(values);
        }
    }
        ...
        navigate(
            route,
            {
                state: {
                    ...
                    origin: 'code-sub-category',  // origin - will return here on save
    ...
    const handleEditClass = (codeCategory, values) => {
        const {id} = codeCategories.filter(cls => cls.codeCategory === codeCategory)[0] || {};
        log(`Editing code category: ${id}`);
        handleEditSettings(id, values, '/code-category');
    }

    const handleDuplicateClass = (codeCategory, values) => {
        const {id} = codeCategories.filter(cls => cls.codeCategory === codeCategory)[0] || {};
        log(`Duplicating code category: ${id}`);
        handleDuplicateSettings(id, values, '/code-category');
    }

    const handleCreateClass = (values) => {
        log(`Creating new code category`);
        handleCreateSettings(values, '/code-category')
    }
    ...
    const codeCategoryOptions = codeCategories.map((_option) => {
        return {
            value: _option.id,
            label: `${_option.codeCategoryNumber} - ${_option.codeCategory}`,
    ...
    return(
        <Box m="20px" minHeight="85vh">
            <Header title="CODE CATEGORIES" subtitle="Create / edit code categories" />

            <Formik
                onSubmit={handleFormSubmit}
                initialValues={getInitialValues(loadId, loadMode)}
                validationSchema={codeSubCategorySchema}
            >
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        >
                            ...
                            <TextField
                                ...
                                variant="filled"
                                type="text"
                                label="Code Category Number"
                                onBlur={handleBlur}
                                onChange={handleChange}
                                value={values.codeSubCategoryNumber}
                                name="codeSubCategoryNumber"
                                error={!!touched.codeSubCategoryNumber && !!errors.codeSubCategoryNumber}
                                helperText={touched.codeSubCategoryNumber && errors.codeSubCategoryNumber}
                                sx={{ gridColumn: "span 4" }}
                            />
                            ...
                            <TextField
                                ...
                                onBlur={handleBlur}
                                onChange={handleChange}
                                value={values.codeSubCategory}
                                name="codeSubCategory"
                                error={!!touched.codeSubCategory && !!errors.codeSubCategory}
                                helperText={touched.codeSubCategory && errors.codeSubCategory}
                                sx={{ gridColumn: "span 4" }}
                            />
                            ...
                            <FormControl sx={{
                                ...
                                <Select
                                    options={codeCategoryOptions}
                                    name="codeCategory"
                                    value={(codeCategoryOptions ? codeCategoryOptions.find(option => option.codeCategory === values.codeCategory) : '')}
                                    onChange={(option) => {
                            ...
                            >
                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleEditClass(values.codeCategory, values) }}>
                                        EDIT
                                    </Button>
                                </Box>

                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleDuplicateClass(values.codeCategory, values) }}>
                                        COPY
                                    </Button>
                                </Box>
...
export default CodeSubCategory;
