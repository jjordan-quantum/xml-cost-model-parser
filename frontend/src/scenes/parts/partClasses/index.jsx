...
const PartClasses = () => {
    ...
    const handleEdit = (id) => {
        ...
        navigate(
            '/part-class',
            { state: { loadId: id, loadMode: 'edit', origin: 'part-classes' } }
        );
    }
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const partClassesClone = [...partClasses];

            for(let i = 0; i < partClassesClone.length; i++) {
                const { id } = partClassesClone[i];
            ...
            if(index !== -1) {
                console.log(partClassesClone);
                partClassesClone.splice(index, 1);
                setPartClasses(partClassesClone);
            }
    ...
    const handleDuplicate = (id) => {
        ...
        navigate(
            '/part-class',
            { state: { loadId: id, loadMode: 'duplicate', origin: 'part-classes' } }
        );
    }

    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR PART CLASSES?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setPartClasses([]);
            window.alert(`Storage for part classes reset back to defaults`);
