...
const RentalTypes = () => {
    ...
    const handleEdit = (id) => {
        ...
        navigate(
            '/rental-type',
            { state: { loadId: id, loadMode: 'edit', origin: 'rental-types' } }
        );
    }
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const rentalTypesClone = [...rentalTypes];

            for(let i = 0; i < rentalTypesClone.length; i++) {
                const { id } = rentalTypesClone[i];
            ...
            if(index !== -1) {
                console.log(rentalTypesClone);
                rentalTypesClone.splice(index, 1);
                setRentalTypes(rentalTypesClone);
            }
    ...
    const handleDuplicate = (id) => {
        ...
        navigate(
            '/rental-type',
            { state: { loadId: id, loadMode: 'duplicate', origin: 'rental-types' } }
        );
    }

    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR RENTAL TYPES?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setRentalTypes([]);
            window.alert(`Storage for rental types reset back to defaults`);
