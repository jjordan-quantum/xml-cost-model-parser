...
const Rental = () => {
    ...
    const [rentalEdits, setRentalEdits] = useSessionStorage('rentalEdits', []);
    const [newRentalEdit, setNewRentalEdit] = useSessionStorage('newRentalEdit', undefined);

    const getValuesForId = (id) => {
        try {
            return rentals.filter(o => (
                o
    ...
    const handleSave = (values) => {
        ...
        try {
            ...
            if(typeof(values.id) !== 'undefined') {
                useRentalId = values.id;
                window.alert(`Rentals updated for preset # ${useRentalId} with description:\n ${values.description}`);
            } else {
                useRentalId = savedId;
                window.alert(`Rentals updated for preset # ${useRentalId} with description:\n ${values.description}`);
            }
            ...
            console.log(e);
            window.alert(`Error saving rental: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const updateEditForId = (editId, values) => {
        ...
        try {
            for(let i = 0; i < rentalEdits.length; i++) {
                if(rentalEdits[i].id === editId) {
                    rentalEdits[i] = {...values};
                    foundEdit = true;
                    break;
                }
            }
        ...
        navigate(
            route,
            {
                state: {
                    ...
                    origin: 'rental',  // origin - will return here on save
    ...
    const handleEditClass = (rentalType, values) => {
        const {id} = rentalTypes.filter(cls => cls.rentalType === rentalType)[0] || {};
        log(`Editing rental type: ${id}`);
        handleEditSettings(id, values, '/rental-type');
    }

    const handleDuplicateClass = (rentalType, values) => {
        const {id} = rentalTypes.filter(cls => cls.rentalType === rentalType)[0] || {};
        log(`Duplicating rental type: ${id}`);
        handleDuplicateSettings(id, values, '/rental-type');
    }

    const handleCreateClass = (values) => {
        log(`Creating new rental type`);
        handleCreateSettings(values, '/rental-type')
    }
    ...
    const rentalTypeOptions = rentalTypes.map((_option) => {
        return {
            value: _option.id,
            label: _option.rentalType,
        }
    });
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        >
                            ...
                            <TextField
                                ...
                                value={values.rental}
                                name="rental"
                                error={!!touched.rental && !!errors.rental}
                                helperText={touched.rental && errors.rental}
                                sx={{ gridColumn: "span 4" }}
                            />
                            ...
                            <FormControl sx={{
                                ...
                                <Select
                                    ...
                                    name="rentalType"
                                    value={(rentalTypeOptions ? rentalTypeOptions.find(option => option.label === values.rentalType) : '')}
                                    onChange={(option) => {
                                        setFieldValue( 'rentalType', option.label, false  );
                                    }}
                                    onBlur={handleBlur}
                                    styles={customStyles}
                                />
                            ...
                            >
                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleEditClass(values.rentalType, values) }}>
                                        EDIT
                                    </Button>
                                </Box>

                                <Box ml="5px" mr="5px">
                                    <Button color="secondary" variant="contained" onClick={() => { handleDuplicateClass(values.rentalType, values) }}>
                                        COPY
                                    </Button>
                                </Box>
