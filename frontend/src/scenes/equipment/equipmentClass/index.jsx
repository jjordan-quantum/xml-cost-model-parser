...
const EquipmentClass = () => {
    const componentName = 'EquipmentClass';
    ...
    const getValuesForId = (id) => {
        try {
            return equipmentClasses.filter(o => (
                o
    ...
    const handleSave = (values) => {
        ...
        const {description, equipmentClass} = values;
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                if(index === -1) {
                    if((equipmentClasses || []).map((p) => p.equipmentClass.toLowerCase()).includes(equipmentClass.toLowerCase())) {
                        log(`equipmentClass must be unique - already have an entry for:\n\n${equipmentClass}`);
                        window.alert(`equipmentClass must be unique - already have an entry for:\n\n${equipmentClass}`);
                        return;
                    }

                    equipmentClasses.push({...values});
                } else {
                    equipmentClasses[index] = {...values};
                }
            } else {
                let maxId = -1;
                ...
                for(const item of equipmentClasses) {
            ...
            setEquipmentClasses(equipmentClasses);
            let useEquipmentClassId;

            if(typeof(values.id) !== 'undefined') {
                useEquipmentClassId = values.id;
                window.alert(`Equipment Classes updated for preset # ${useEquipmentClassId} with description:\n ${values.description} and class ${equipmentClass}`);
            } else {
                useEquipmentClassId = savedId;
                window.alert(`Equipment Classes updated for preset # ${useEquipmentClassId} with description:\n ${values.description} and class ${equipmentClass}`);
            }

            return useEquipmentClassId;
        } catch(e) {
            log(`Error saving equipment class`);
            console.log(e);
            window.alert(`Error saving equipment class: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useEquipmentClassId = handleSave(values);

        if(typeof(useEquipmentClassId) !== 'undefined') {
            _navigate(useEquipmentClassId);
        }
    }
    ...
    return(
        <Box m="20px" minHeight="85vh">
            <Header title="EQUIPMENT CLASS" subtitle="Create / edit equipment classes" />

            <Formik
                onSubmit={handleFormSubmit}
                initialValues={getInitialValues(loadId, loadMode)}
                validationSchema={equipmentClassSchema}
            >
