...
const EquipmentClasses = () => {
    ...
    const handleEdit = (id) => {
        ...
        navigate(
            '/equipment-class',
            { state: { loadId: id, loadMode: 'edit', origin: 'equipment-classes' } }
        );
    }
    ...
    const handleDelete = (_id, description) => {
        ...
        if(window.confirm(`Are you sure you want to delete the preset ${_id} with description: \n '${description}'?`)) {
            let index = -1;
            const equipmentClassesClone = [...equipmentClasses];

            for(let i = 0; i < equipmentClassesClone.length; i++) {
                const { id } = equipmentClassesClone[i];
            ...
            if(index !== -1) {
                console.log(equipmentClassesClone);
                equipmentClassesClone.splice(index, 1);
                setEquipmentClasses(equipmentClassesClone);
            }
    ...
    const handleDuplicate = (id) => {
        ...
        navigate(
            '/equipment-class',
            { state: { loadId: id, loadMode: 'duplicate', origin: 'equipment-classes' } }
        );
    }

    const handleClearPresets = () => {
        if(window.confirm(`ARE YOU SURE YOU WANT TO CLEAR BROWSER STORAGE FOR EQUIPMENT CLASSES?\n\nTHIS WILL DELETE ALL PRESETS.`)) {
            setEquipmentClasses([]);
            window.alert(`Storage for equipment classes reset back to defaults`);
    ...
    return (
        <Box m="20px">
            <Header title="EQUIPMENT CLASSES" subtitle="List of existing presets for Equipment Classes"/>
            <Box display="flex" justifyContent="left" mt="10px" p="10px">
                <Button type="submit" color="secondary" variant="contained" sx={{ ml: 1 }} onClick={() => {
                    navigate(
                        '/equipment-class',
                        { state: { loadMode: 'new', origin: 'equipment-classes' } }
                    ) ;
                }}>
                    Create New Preset
                </Button>
