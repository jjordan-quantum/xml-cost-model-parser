...
import incidentalItemSchema from "../../../data/schemas/incidentalItemSchema";
...
const IncidentalItem = () => {
    const componentName = 'IncidentalItem';
    ...
    const getValuesForId = (id) => {
        try {
            return incidentalItems.filter(o => (
                o
    ...
    const handleSave = (values) => {
        ...
        const {description, projectNumber, incidentalItemNumber} = values;
        ...
        if(!incidentalItemNumber) {
            log(`incidentalItemNumber undefined - cannot save settings`);
            window.alert(`incidentalItemNumber undefined - cannot save settings`);
            return;
        }
        ...
        try {
            if(typeof(values.id) !== 'undefined') {
                ...
                for(let i = 0; i < incidentalItems.length; i++) {
                    const { id } = incidentalItems[i];
                ...
                if(index === -1) {
                    if((incidentalItems || []).map((p) => (
                        p.projectNumber === projectNumber
                        && p.incidentalItemNumber === incidentalItemNumber
                    ))[0]) {
                        log(`incidentalItem must be unique - already have an entry for:\n\n${projectNumber} and ${incidentalItemNumber}`);
                        window.alert(`incidentalItem must be unique - already have an entry for:\n\n${projectNumber} and ${incidentalItemNumber}`);
                        return;
                    }

                    incidentalItems.push({...values});
                } else {
                    incidentalItems[index] = {...values};
                }
            } else {
                let maxId = -1;
                ...
                for(const item of incidentalItems) {
            ...
            setIncidentalItems(incidentalItems);
            let useIncidentalItemId;

            if(typeof(values.id) !== 'undefined') {
                useIncidentalItemId = values.id;
                window.alert(`Incidental Items updated for preset # ${useIncidentalItemId} with description:\n ${values.description} and project ${projectNumber}`);
            } else {
                useIncidentalItemId = savedId;
                window.alert(`Incidental Items updated for preset # ${useIncidentalItemId} with description:\n ${values.description} and project ${projectNumber}`);
            }

            return useIncidentalItemId;
        } catch(e) {
            log(`Error saving incidental item`);
            console.log(e);
            window.alert(`Error saving incidental item: ${e.toString()}`);
            return undefined;
        }
    }
    ...
    const handleFormSubmit = (values) => {
        ...
        const useBidItemId = handleSave(values);

        if(typeof(useBidItemId) !== 'undefined') {
            _navigate(useBidItemId);
        }
    }
    ...
    return(
        <Box m="20px" minHeight="85vh">
            <Header title="BID ITEM" subtitle="Create / edit bid items" />

            <Formik
                onSubmit={handleFormSubmit}
                initialValues={getInitialValues(loadId, loadMode)}
                validationSchema={incidentalItemSchema}
            >
                        ...
                        >
                            ...
                            <FormControl sx={{
                                ...
                                <Select
                                    ...
                                    name="projectNumber"
                                    value={(projectOptions ? projectOptions.find(option => option.id === values.projectNumber) : '')}
                                    onChange={(option) => {
                                        setFieldValue( 'projectNumber', option.id, false  );
                                    }}
                                    onBlur={handleBlur}
                                    styles={customStyles}
                                />
                            ...
                            <TextField
                                ...
                                variant="filled"
                                type="text"
                                label="Incidental Item Number"
                                onBlur={handleBlur}
                                onChange={handleChange}
                                value={values.incidentalItemNumber}
                                name="incidentalItemNumber"
                                error={!!touched.incidentalItemNumber && !!errors.incidentalItemNumber}
                                helperText={touched.incidentalItemNumber && errors.incidentalItemNumber}
                                sx={{ gridColumn: "span 4" }}
                            />
                            ...
                            <TextField
                                ...
                                onBlur={handleBlur}
                                onChange={handleChange}
                                value={values.bidItemCode}
                                name="bidItemCode"
                                error={!!touched.bidItemCode && !!errors.bidItemCode}
                                helperText={touched.bidItemCode && errors.bidItemCode}
                                sx={{ gridColumn: "span 4" }}
                            />
